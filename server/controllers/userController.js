...
// ✅ Follow user — Instagram style (pending approval)
export const followUser = async (req, res) => {
  try {
    ...
    if (existingRequest) {
      return res.json({ success: false, message: "Follow request already sent" });
    }
...
export const unfollowUser = async (req, res) => {
  try {
    ...
    // pending request bhi delete karo agar hai
...
export const getUserConnections = async (req, res) => {
  try {
    ...
    // pending follow requests — jo mujhe aaye hain
    ...
    // pending connection requests — jo mujhe aaye hain
    ...
    res.json({
      ...
      pendingConnections: [...pendingFollowRequests, ...pendingConnectionRequests],
    });
  } catch (error) {
    console.log(error);
    res.json({ success: false, message: error.message });
...
// ✅ Accept request — follow ya connection dono handle karta hai
export const acceptConnectionRequest = async (req, res) => {
  try {
    const { userId } = req.auth();
    const { id } = req.body;

    // pehle follow request dhundo
    const followRequest = await Connection.findOne({
      from_user_id: id,
      to_user_id: userId,
      type: "follow",
      status: "pending",
    ...
    if (followRequest) {
      // sirf requester ki following mein add karo — one way
      await User.findByIdAndUpdate(id, { $addToSet: { following: userId } });
    ...
    // dono ke connections mein add karo — two way
