...
const CreatePost = () => {
  ...
  const generateHashtags = async () => {
    ...
    try {
      ...
      toast.error("Hashtags generate nahi hue. Dobara try karo!");
    }
  ...
  return (
    <div className="h-full overflow-y-auto no-scrollbar bg-transparent">
      <div className="max-w-xl mx-auto px-6 py-8">
        ...
        <div
          className={`rounded-2xl border overflow-hidden ${isDark ? "bg-zinc-900 border-white/5" : "bg-white border-slate-100 shadow-sm"}`}
        >
          {/* User info */}
          <div
            className={`flex items-center gap-3 px-5 pt-5 pb-4 border-b ${isDark ? "border-white/5" : "border-slate-50"}`}
          >
            <img
          ...
          <div
            className={`mx-5 mb-4 rounded-xl border overflow-hidden ${isDark ? "border-white/5 bg-zinc-800/50" : "border-slate-100 bg-slate-50"}`}
          >
            <div className="flex items-center justify-between px-4 py-3">
              <div className="flex items-center gap-2">
                <Sparkles
                  className={`w-4 h-4 ${isDark ? "text-sky-400" : "text-sky-500"}`}
                />
                <span
                  className={`text-xs font-semibold ${isDark ? "text-white/60" : "text-slate-600"}`}
                >
              ...
              <div className="px-4 pb-3 flex gap-2">
                <input
                  ...
                  className={`flex-1 text-xs px-3 py-2 rounded-lg outline-none border ${
                    isDark
                      ? "bg-zinc-900 border-white/10 text-white placeholder-white/20"
              ...
              >
                {aiLoading ? (
                  ...
                  <div className="p-2 space-y-1.5">
                    <p
                      className={`text-[10px] px-2 pb-1 ${isDark ? "text-white/20" : "text-slate-400"}`}
                    >
                      Click karo to use ✨
                    </p>
                    {aiSuggestions.map((caption, i) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          toast.success("Caption apply ho gaya!");
                        }}
          ...
          {images.length > 0 && (
            <div
              className={`mx-5 mb-4 rounded-xl border overflow-hidden ${isDark ? "border-violet-500/10 bg-violet-500/5" : "border-violet-100 bg-violet-50/50"}`}
            >
              <div className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-2">
                  <Image
                  ...
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded-full ${isDark ? "bg-violet-500/10 text-violet-400" : "bg-violet-100 text-violet-500"}`}
                  >
              ...
              {showImgSuggestions && (
                <div
                  className={`border-t ${isDark ? "border-violet-500/10" : "border-violet-100"}`}
                >
                  {imgCaptionLoading ? (
                    <div className="flex items-center justify-center gap-2 py-6">
                      <Loader2
                        className={`w-4 h-4 animate-spin ${isDark ? "text-violet-400/50" : "text-violet-300"}`}
                      />
                      <span
                        className={`text-xs ${isDark ? "text-white/30" : "text-slate-400"}`}
                      >
                        Photo dekh ke likh raha hai...
                      </span>
                    </div>
                  ) : (
                    <div className="p-2 space-y-1.5">
                      ...
                      {imgCaptionSuggestions.map((caption, i) => (
                        <button
                          ...
                          className={`w-full text-left text-xs px-3 py-2.5 rounded-lg transition cursor-pointer ${
                            isDark
                              ? "text-white/70 hover:bg-violet-500/10 hover:text-white"
          ...
          <div
            className={`mx-5 mb-4 rounded-xl border overflow-hidden ${isDark ? "border-emerald-500/10 bg-emerald-500/5" : "border-emerald-100 bg-emerald-50/50"}`}
          >
            <div className="flex items-center justify-between px-4 py-3">
              <div className="flex items-center gap-2">
                <Hash
            ...
            {showHashtags && (
              <div
                className={`border-t ${isDark ? "border-emerald-500/10" : "border-emerald-100"}`}
              >
                {hashtagLoading ? (
                  <div className="flex items-center justify-center gap-2 py-6">
                    <Loader2
                      className={`w-4 h-4 animate-spin ${isDark ? "text-emerald-400/50" : "text-emerald-300"}`}
                    />
                    <span
                      className={`text-xs ${isDark ? "text-white/30" : "text-slate-400"}`}
                    >
                      Hashtags dhundh raha hai...
                    </span>
                  </div>
                ) : (
          ...
          {/* Bottom bar */}
          <div
            className={`flex items-center justify-between px-5 py-3 border-t ${isDark ? "border-white/5 bg-zinc-950/50" : "border-slate-50 bg-slate-50/50"}`}
          >
            <label
              htmlFor="images"
