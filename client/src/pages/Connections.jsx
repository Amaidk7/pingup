      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {dataArray.map((item) => (
            ...
            >
              <item.icon
                className={`w-4 h-4 mb-1 ${currentTab === item.label ? (isDark ? "text-black/60" : "text-white/60") : isDark ? "text-white/20" : "text-slate-300"}`}
              />
              <span className="text-xl font-bold">
                {item.value?.length || 0}
              </span>
              <span
                className={`text-xs ${currentTab === item.label ? (isDark ? "text-black/60" : "text-white/60") : isDark ? "text-white/30" : "text-slate-400"}`}
              >
                {item.label}
              </span>
            </button>
        ...
        {/* Tabs */}
        <div
          className={`flex flex-wrap items-center gap-1 rounded-2xl p-1 mb-6 w-fit border ${isDark ? "bg-zinc-900 border-white/5" : "bg-white border-slate-100 shadow-sm"}`}
        >
          {dataArray.map((tab) => (
            <button
              key={tab.label}
        ...
        <div className="space-y-2">
          {currentData.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-3 ${isDark ? "bg-white/5" : "bg-slate-100"}`}
              >
                <Users
                  className={`w-5 h-5 ${isDark ? "text-white/20" : "text-slate-300"}`}
                />
              </div>
              <p
          ...
          {currentData.map((user) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p
                  className={`text-xs truncate ${isDark ? "text-white/30" : "text-slate-400"}`}
                >
                  @{user.username}
                </p>
                {user.bio && (
                  <p
                    className={`text-xs mt-0.5 truncate ${isDark ? "text-white/20" : "text-slate-500"}`}
                  >
                    {user.bio?.slice(0, 50)}
                  </p>
                )}
              ...
              <div className="flex gap-2 shrink-0">
                ...
                {currentTab === "Pending" && (
                  <button
                    ...
                    className={`px-3 py-2 text-xs font-semibold border rounded-xl active:scale-95 transition cursor-pointer ${
                      isDark
                        ? "text-sky-400 border-sky-500/30 hover:border-sky-400"
                        : "text-sky-600 border-sky-200 hover:border-sky-400"
                    }`}
                  >
                    Accept
                  </button>
