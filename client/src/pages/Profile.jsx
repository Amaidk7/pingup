...
const Profile = () => {
  ...
  return user ? (
    <div
      className={`relative h-full overflow-y-scroll no-scrollbar bg-transparent`}
    >
      <div className="max-w-2xl mx-auto pb-10 px-4 sm:px-6">
        ...
        <div className="mt-6">
          <div
            className={`flex items-center gap-1 rounded-2xl p-1 border ${isDark ? "bg-zinc-900 border-white/5" : "bg-white border-slate-100 shadow-sm"}`}
          >
            {tabs.map((tab) => (
              <button
                key={tab}
            ...
            <div className="mt-5 space-y-4">
              {posts?.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  <p
                    className={`text-sm ${isDark ? "text-white/20" : "text-slate-400"}`}
                  >
                    No posts yet
                  </p>
                </div>
