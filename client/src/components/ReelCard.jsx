  ...
  return (
    <div className="relative w-full h-screen flex items-center justify-center bg-black snap-start snap-always">
      ...
      <div className="absolute bottom-6 left-4 right-16 space-y-3">
        ...
        >
          <img
            ...
            alt=""
            className="w-9 h-9 rounded-full object-cover border-2 border-sky-400/50"
          />
          <div>
      ...
      <div className="absolute bottom-6 right-4 flex flex-col items-center gap-5">
        ...
        >
          <div
            className={`w-10 h-10 flex items-center justify-center rounded-full transition ${isLiked ? "bg-rose-500/20" : "bg-white/10 hover:bg-white/20"}`}
          >
            <Heart
              className={`w-5 h-5 transition-transform group-hover:scale-110 ${isLiked ? "fill-rose-400 text-rose-400" : "text-white"}`}
            />
          </div>
