...
const Notification = ({ t, message }) => {
  ...
  return (
    <div className={`max-w-sm w-full rounded-2xl overflow-hidden transition-all duration-300 border shadow-xl ${
      isDark
      ...
      <div className="flex items-center gap-3 p-4">
        <div className="relative shrink-0">
          ...
          <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-emerald-400 rounded-full border-2 ${isDark ? "border-zinc-900" : "border-white"}`}></span>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-xs truncate mt-0.5 ${isDark ? "text-white/40" : "text-slate-400"}`}>
            {message?.text?.slice(0, 45)}{message?.text?.length > 45 ? "..." : ""}
          </p>
        </div>
