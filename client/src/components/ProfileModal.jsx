...
const ProfileModal = ({ setShowEdit }) => {
  ...
  const generateBio = async () => {
    ...
    try {
      ...
      let bios = [];
      if (text.includes("|||")) {
        bios = text.split("|||").map((b) => b.trim()).filter(Boolean);
      } else {
        bios = text
          .split("\n")
        ...
        <form className="p-6 space-y-5"
          ...
          <div className="pt-8 space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button
                  ...
                  className={`flex items-center gap-1 text-[10px] font-semibold px-2.5 py-1 rounded-lg transition active:scale-95 cursor-pointer ${
                    isDark ? "bg-sky-500/10 text-sky-400 hover:bg-sky-500/20 border border-sky-500/20" : "bg-sky-50 text-sky-600 hover:bg-sky-100 border border-sky-100"
                  }`}>
                  {aiLoading
              ...
              {/* AI Suggestions */}
              {showSuggestions && (
                <div className={`mt-2 rounded-xl border overflow-hidden ${isDark ? "border-white/5 bg-zinc-800/50" : "border-slate-100 bg-slate-50"}`}>
                  {aiLoading ? (
                    ...
                    <div className="p-2 space-y-1">
                      <p className={`text-[10px] px-2 py-1 ${isDark ? "text-white/20" : "text-slate-400"}`}>Click karo to use ✨</p>
